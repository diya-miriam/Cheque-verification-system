...
def verify_signature(roi_sig_pil: Image.Image, ref_sig_pil: Image.Image):
    ...
    # Cheque ROI already came from YOUR preprocessing pipeline
    cheque_np = np.array(roi_sig_pil.convert("L"), dtype=np.uint8)
    cheque_sig_tensor = roi_to_model_tensor(cheque_np)

    # Reference signature does NOT go through cheque preprocessing pipeline
    # It only follows the same final preparation used in training dataset
